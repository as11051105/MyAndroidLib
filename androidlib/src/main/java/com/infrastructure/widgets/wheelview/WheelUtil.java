 ...
 * @author Mr.Z
 * @version 1.0
 * @dateTime 2015-2-9 上午11:24:32
 */
public class WheelUtil {
    /**
     * 描述：默认的年月日的日期选择器.
     *
     * @param mTimeView     the m mTimeView
    ...
    public static void initWheelDatePicker(final Context ctx, View mTimeView, final TextView mText, final WheelView mWheelViewY, final WheelView mWheelViewM,
        ...
        mWheelViewM.setCyclic(false);
        mWheelViewM.setLabel("月");
        mWheelViewM.setVisibleItems(7);
     ...
     */
    public static void initWheelTimePicker(final Activity activity, final TextView mText, final WheelView mWheelViewMD, final WheelView mWheelViewHH, final WheelView mWheelViewMM, Button okBtn, Button cancelBtn, int defaultYear, int defaultMonth, int defaultDay, int defaultHour, int defaultMinute, boolean initStart) {
        Calendar calendar = Calendar.getInstance();
        int year = calendar.get(Calendar.YEAR);
        int month = calendar.get(Calendar.MONTH) + 1;
        int day = calendar.get(Calendar.DATE);
        ...
        for (int i = 1; i < 13; i++) {
            if (list_big.contains(String.valueOf(i))) {
                ...
                if (i == 2) {
                    if (TimeUtils.isLeapYear(defaultYear)) {
                        for (int j = 1; j < 28; j++) {
                            textDMList.add(i + "月" + " " + j + "日");
                            textDMDateList.add(defaultYear + "-" + i + "-" + j);
                        }
        ...
        // 月日
        mWheelViewMD.setAdapter(new StringWheelAdapter(textDMList, StringUtils.strLength("12月" + " " + "12日")));
     ...
     * @param initStart     the init start
     */
    public static void initWheelTimePicker2(final Activity activity, final TextView mText, final WheelView mWheelViewHH, final WheelView mWheelViewMM, Button okBtn, Button cancelBtn, int defaultHour, int defaultMinute, boolean initStart) {
        Calendar calendar = Calendar.getInstance();
        int hour = calendar.get(Calendar.HOUR_OF_DAY);
        int minute = calendar.get(Calendar.MINUTE);
        ...
        // }
        //
        // String val = StrUtil.dateTimeFormat(defaultHour + ":" +
        // defaultMinute);
        // mText.setText(val);
        ...
        mWheelViewHH.setLabel("点");
        mWheelViewHH.setCurrentItem(defaultHour);
        mWheelViewHH.setVisibleItems(7);
        // mWheelViewH.setCenterSelectDrawable(this.getResources().getDrawable(R.drawable.wheel_select));
        ...
        mWheelViewMM.setCyclic(true);
        mWheelViewMM.setLabel("分");
        mWheelViewMM.setVisibleItems(7);
        mWheelViewMM.setCurrentItem(defaultMinute);
        // mWheelViewM.setCenterSelectDrawable(this.getResources().getDrawable(R.drawable.wheel_select));
