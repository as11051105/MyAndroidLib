...
class TrailerInputStream extends InputStream {
     ...
     * @param trailerSize the byte length of the trailer
     */
     ...
     *
     * @throws EOFException if the stream finishes before the trailer is read
    ...
    private void fillTrailerBuffer() throws IOException {
        ...
        if (bytesRead != trailerBuffer.length) {
            throw new EOFException(String.format("Trailer size was %d bytes but stream only contained %d bytes.", trailerSize, bytesRead));
        }
    }
     ...
     *
     * @return the next non-trailer byte from the underlying stream, or
     * {@code -1}
     * @throws EOFException if the stream contains less data than the size of the trailer
     * @throws IOException  if the underlying stream throws an exception
     */
    @Override
    public int read() throws IOException {
        ...
        System.arraycopy(trailerBuffer, 1, trailerBuffer, 0, trailerBuffer.length - 1);
     ...
     *                     there are insufficient bytes in the underlying stream to read
     *                     the trailer, or if some other I/O error occurs.
     */
    @Override
    public int read(byte[] b, int off, int len) throws IOException {
        ...
        if (numBytesRead <= trailerSize) {
            ...
            System.arraycopy(trailerBuffer, numBytesRead, trailerBuffer, 0, trailerSize - numBytesRead);

            // Now need to fill rear of trailer
            System.arraycopy(inputBuffer, 0, trailerBuffer, trailerSize - numBytesRead, numBytesRead);
            ...
            System.arraycopy(inputBuffer, numBytesRead - trailerSize, trailerBuffer, 0, trailerSize);
        }
     ...
     *
     * @return the trailer data, or {@code null} if no bytes have been read
     * using the {@code read} methods
     */
