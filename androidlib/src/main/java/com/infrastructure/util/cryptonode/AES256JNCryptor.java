...
package com.infrastructure.util.cryptonode;
 ...
 * </p>
 * See <a href="https://github.com/rnapier/RNCryptor/wiki/Data-Format">https://
 * github .com/rnapier/RNCryptor/wiki/Data-Format</a>, from which most of the
...
public class AES256JNCryptor implements JNCryptor {
    ...
    /**
     * Creates a new {@code AES256JNCryptor} instance. Uses the default number
     * of PBKDF iterations.
     */
    ...
    /**
     * Creates a new {@code AES256JNCryptor} instance that uses a specific
     * number of PBKDF iterations.
     *
     * @param iterations the number of PBKDF iterations to perform
     */
    ...
    /**
     * Returns random data supplied by this class' {@link SecureRandom}
     * instance.
     *
    ...
    private static int readVersionNumber(byte[] data) {
        Validate.isTrue(data.length > 0, "Data must be at least one byte long to read version number.");

        return data[0];
    }

    /**
     * Compares arrays for equality in constant time. This avoids certain types
     * of timing attacks.
     *
    ...
    public SecretKey keyForPassword(char[] password, byte[] salt) throws CryptorException {
        ...
        try {
            ...
            SecretKey tmp = factory.generateSecret(new PBEKeySpec(password, salt, getPBKDFIterations(), AES_256_KEY_SIZE * 8));
     ...
     * Decrypts data.
     *
     * @param aesCiphertext the ciphertext from the message
     * @param decryptionKey the key to decrypt
     * @param hmacKey       the key to recalculate the HMAC
     * @return the decrypted data
     * @throws CryptorException if a JCE error occurs
     */
    private byte[] decryptV2Data(AES256v2Ciphertext aesCiphertext, SecretKey decryptionKey, SecretKey hmacKey) throws CryptorException {
        ...
        try {
            ...
            cipher.init(Cipher.DECRYPT_MODE, decryptionKey, new IvParameterSpec(aesCiphertext.getIv()));
    ...
    public byte[] decryptData(byte[] ciphertext, char[] password) throws CryptorException {
        ...
        // I don't like the magic numbers here, but can't think of a pleasant
        // way
        // to solve this
    ...
    private byte[] decryptV2Data(byte[] ciphertext, char[] password) throws CryptorException {
        try {
            ...
            SecretKey decryptionKey = keyForPassword(password, aesCiphertext.getEncryptionSalt());
    ...
    public byte[] encryptData(byte[] plaintext, char[] password, byte[] encryptionSalt, byte[] hmacSalt, byte[] iv) throws CryptorException {
        ...
        return encryptData(plaintext, new PasswordKey(encryptionKey, encryptionSalt), new PasswordKey(hmacKey, hmacSalt), iv);
    }
    ...
    @Override
    public byte[] decryptData(byte[] ciphertext, SecretKey decryptionKey, SecretKey hmacKey) throws CryptorException {
        ...
        try {
            ...
            switch (version) {
                case 2:
                    return decryptV2Data(new AES256v2Ciphertext(ciphertext), decryptionKey, hmacKey);
                case 3:
                    return decryptV3Data(new AES256v3Ciphertext(ciphertext), decryptionKey, hmacKey);
                default:
                    throw new CryptorException(String.format("Unrecognised version number: %d.", version));
            }
