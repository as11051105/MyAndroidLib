...
/**
 * Reads RNCryptor-format data in a stream fashion. This class only supports the
 * v3 data format. The entire stream must be read in order to trigger the
 * validation of the HMAC value.
 *
 * @since 1.1.0
 */
public class AES256JNCryptorInputStream extends InputStream {
     ...
     *
     * @param in            the {@code InputStream} to read
     * @param decryptionKey the key to decrypt with
     * @param hmacKey       the key to calculate the HMAC with
     */
     ...
     * @throws EOFException if we run out of data before reading the header
     */
    private void initializeStream() throws IOException {
        ...
        StreamUtils.readAllBytesOrFail(in, headerData); // throws EOF if
        // insufficient data
        ...
        try {
            ...
            // The decryption stream will write the non-decrypted bytes to the
            // mac
            // stream
            pushbackInputStream = new PushbackInputStream(new CipherInputStream(new MacUpdateInputStream(trailerIn, mac), decryptCipher), 1);
    ...
    /**
     * Reads the next byte from the input stream. If this is the last byte in
     * the stream (determined by peeking ahead to the next byte), the value of
    ...
    /**
     * The {@code read(b)} method for class {@code AES256JNCryptorInputStream}
    ...
    /**
     * Reads a number of bytes into the byte array. If this includes the last
     * byte in the stream (determined by peeking ahead to the next byte), the
     * value of the HMAC is verified. If the verification fails an exception is
     * thrown.
     *
     * @param b   the buffer into which the data is read.
     ...
     *
     * @param b the result of a read operation
     * @return the value {@code b}
     */
    private int completeRead(int b) throws IOException {
        if (b == END_OF_STREAM) {
            handleEndOfStream();
        } else {
     ...
     *
     * @throws IOException if the HMAC value is incorrect
     */
